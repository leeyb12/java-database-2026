...
- 관계형 데이터베이스(RDBMS)
  - `Oracle` - 학습할 DB
  - SOLServer - Microsoft사 제품. Oracle보다 성능이 낮음
...
2. Docker Command 사용
    ...
    - 컨테이너 자동실행 명령
        ...
        ```bash
        docker update --restart=always [도커ID]
        ```
    ...
    - 컨테이너 내부 접속
        ...
        SQL*Plus: Release 21.0.0.0.0 - Production on Thu Feb 26 03:19:03 2026
        Version 21.3.0.0.0

        Copyright (c) 1982, 2021, Oracle.  All rights reserved.

        Last Successful login time: Mon Sep 30 2024 01:54:37 +00:00
    ...
    - 강의용 사용자 생성
        ...
        GRANT all prinvileges TO java; -- INSERT를 추가 권한 할당
        ```
...
- Database Navigator에서 DB연결 시작
    ...
    - 마우스 오른쪽버튼 > Create > Connection
        ...
        ![alt text](./day01/img/image-6.png)
...
- 오라클 함수 : DB별로 추가학습 필요 - [쿼리](./day02/8.오라클함수.sql)
    ...
    - 문자열 함수
        ...
        - `TRIM`(공백이있는문자열), `LTRIM`(공백있는문자열), `RTRIM`(공백있는문자열), 문자열 앞뒤의 빈공백 제거
...
- 관계형 데이터베이스
    ...
    - 컬럼 - 데이터 특징을 담는 하난의 속성, Column/Attribute
...
- 조인 계속 - [쿼리](./day04/1.조인다시.sql)
    ...
    - 셀프 조인 - 자체조인. 자기 테이블을 조인. 자기 테이블내에 해당 PK와 관련있는 FK가 지정되어 있어야 
...
- 트랜잭션 명령어 - Oracle은 트랜잭션을 시작하는 `BEGIN TRAN[SACTION]` 이 없음  
    - 첫번째 쿼리부터 트랜잭션이 시작됨
    - DBeaver의 경우 메뉴 데이터베이스 > 트랜잭션 모드 > `Manual Commit`으로 변경
    - 환경 설정 > 연결 > 연결 유형 > `Auto-commit by default`를 해제

    ![alt text](./day05/img/image-1.png)

    ![alt text](./day05/img/image.png)
...
- DDL 명령어 - [쿼리](./day05/5.DDL.sql)
    ...
    - `TRUNCATE` - 객체 내 데이터 모두 삭제. 대부분 테이블에서 진행
        ```sql
        -- DELETE FROM 테이블명과 동일. 단, 트랜잭션이 발생하지 않아 복구불가
        -- 테이블 생성 이후 상태가 됨
        TRUNCATE TABLE 테이블명
        ```
...
- 인덱스
    ...
    - 보통 SELECT WHERE절에서 자주 필터링 되는 컬럼에 인덱스를 걸명 속도개선
...
- 사용자 생성 - [쿼리](./day07/1.사용자_권한.sql)
    ...
    CREATE USER 사용자명
    IDENTIFIED BY 패스워드  -- 패스워드는 대소문자구분! 
...
- 예외처리 - [쿼리](./day07/6.예외처리.sql)
    ...
    ![alt text](./day07/img/image-1.png)
...
- 개념 데이터 모델링
    ...
    - 추상화 ERD(Entity Relationship Diagram)를 작성
        ...
        ![alt text](./day08/img/image-2.png)

- 논리 데이터 모델링

    - 개념 데이터 모델링 바탕으로 속성, 키, 관계 명확히 정의
    - 데이터 중복을 최소히 하기위한 **정규화** 수행
    - 관계형 데이터 모델(테이블)로 구체화
    - 논리(물리와 대칭) ERD 작성

        ![alt text](./day08/img/image-3.png)
...
- 물리 데이터 모델링
    ...
    - 최종 스키마 완성
        ...
        ![alt text](./day08/img/image-4.png)
...
- 종류
    ...
    - `erdcloud` - https://www.erdcloud.com/ 모델링이 물리 스키마까지 생성가능한 한국어 지원도구
...
- DB설계자의 개념설계
    - 개체 추출
        - 진료과 아래, 과번호(PK), 속성 - 과이름, 전화번호, 팩스번호
...
- 논리설계 이전 기본키/외래키 정립
    ...
    - 의사 - `의사번호`, 이름, 진료과목, 주소, 전화번호, ~~주민번호~~, 직급, 과번호, ~~의사약력~~
    - 간호사 - `간호사번호`, 이름, 진료과목, 주소, 전화번호, ~~주민번호~~, 직급, 과번호
...
1. ERD 생성 클릭
    ...
    ![alt text](./day08/img/image-5.png)

2. 새로운 엔터티 추가
    - 노란색 + : 키 추가
    - 하늘색 + : 컬럼 추가

    ![alt text](./day09/img/image.png)
...
3. 관계(없거나 한개 또는 여러개) 클릭
    ...
    - 비식별관계는 단순 FK - NOT NULL 또는 NULL 모두 허용. 단, NOT NULL일 경우 데이터 처리시 어려움 발생
        ...
        ![alt text](./day09/img/image-1.png)

    - 결과

        ![alt text](./day09/img/image-2.png)

4. 각 엔터티와 키와 속성 관계로 논리 다이어그램 작성

    ![alt text](./day09/img/image-3.png)
...
6. 내보내기 클릭
    ...
    ![alt text](./day09/img/image-5.png)

7. DB툴에서 복사한 쿼리로 실행

    ![alt text](./day09/img/image-6.png)
...
- 관계에서 부모쪽 연결, 1(NOT NULL)과 0..1(NULL)의 차이 파악할 것
    ...
    ![alt text](./day09/img/image-7.png)
